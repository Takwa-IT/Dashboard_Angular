...
export class ClientsComponent {
  ...
  showEditablePopup(existingData: { name: string; time: string; phone: string; city: string; status: string }) {
    Swal.fire({
      ...
      html: `
        ...
        <input id="swal-input4" class="swal2-input" placeholder="Product Status" value="${existingData.status}">
        <input id="swal-input5" class="swal2-input" placeholder="Enter Time" value="${existingData.time}">
      `,
      focusConfirm: false,
  ...
  searchClient(): void {
    ...
    this.clients = this.clients.filter(
      (client) =>
        ...
        client.time.toLowerCase().includes(query) ||
        client.phone.toString().toLowerCase().includes(query) ||
  ...
  onSearchKeydown(event: KeyboardEvent): void {
    if (event.key === 'Enter') {
      this.searchClient(); // Trigger search when Enter is pressed
    }
  }
